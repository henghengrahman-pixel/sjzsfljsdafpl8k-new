...
export class UpstreamClient{
  ...
  async pendingMoney(type){
    ...
    const r=await this.request(`/agen_playermoneyx.php?action=${action}`);
    if(!r.ok) throw new Error(`UPSTREAM_HTTP_${r.status}`);
    const parsed=parsePendingMoney(r.html,type);
    if(!parsed.actionPath){
      try{const u=new URL(r.url);if(u.pathname==='/agen_playermoneyx.php')parsed.actionPath=u.pathname+u.search}catch{}
    }
    if(!parsed.actionPath) parsed.actionPath=`/agen_playermoneyx.php?action=${action}`;
