...
function parseSearch(html){
  ...
  $('form[action*="editplayerlist.php"]').each((_,form)=>{
    const user=$(form).find('input[name="user"]').attr('value'); if(!user) return;
    const tr=$(form).closest('tr'); const cells=tr.find('td').map((__,el)=>clean($(el).text())).get();
    players.push({userid:user,cells});
  });
  const uniq=[]; const seen=new Set();
  for(const p of players){if(!seen.has(p.userid)){seen.add(p.userid);uniq.push(p);}}
  return uniq;
}
...
export class UpstreamClient{
  ...
  async searchPlayer(userid){
    const r=await this.request('/agentplayerlist.php',{method:'POST',form:{usercheck:'1',user:userid,statusnya:'',banknya:'',name:'',rek:'',phone:'',page:'1',cari:'Cari'}});
    return {http:r.status,players:parseSearch(r.html)};
  }
