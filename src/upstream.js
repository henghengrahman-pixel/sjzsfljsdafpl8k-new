...
export function parsePendingMoney(html,type){
  ...
  form.find('tr').each((_,tr)=>{
    ...
    rows.push({
      key:checkboxName,checkboxName,id2Name,userid,date:clean($(cells[3]).text()),amount,balance,bank,info,
...
export class UpstreamClient{
  ...
  async pendingMoney(type){
    const action=type==='deposit'?'1':'2';
    // AGWL5 protects Deposit/Withdraw with a rotating `code`. Requesting
    // ?action=1/2 without that token can return HTTP 200 but an empty list.
    // Always discover the current token from the authenticated money page.
    const landing=await this.request('/agen_playermoneyx.php');
    if(!landing.ok) throw new Error(`UPSTREAM_HTTP_${landing.status}`);
    let path=discoverMoneyPath(landing.html,type);
    if(!path){
      // If the landing page itself is already the requested lane, retain it
      // only when its hidden action agrees. Never invent a rotating code.
      const $=cheerio.load(landing.html);
      const lane=String($('#action_trans').attr('value')||$('input[name="action"]').first().attr('value')||'');
      if(lane===action){
        try{const u=new URL(landing.url); if(u.pathname==='/agen_playermoneyx.php' && u.searchParams.get('code')) path=u.pathname+u.search;}catch{}
      }
    }
    if(!path) throw new Error('UPSTREAM_MONEY_CODE_NOT_FOUND');
