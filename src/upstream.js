...
export function parsePendingMoney(html,type){
  ...
  form.find('tr').each((_,tr)=>{
    ...
    let destination='',destinationBankName='',destinationAccountNumber='',destinationAccountName='';
    if(type==='deposit' && info){
      // Deposit filtering must follow the receiving bank (To:), never the sender (From:).
      // Stop at a following From: marker if an upstream variant changes ordering.
      // Upstream often collapses <br> tags into text such as "(DANA)To : BCA,..."
      // (no whitespace before To). Parse the last explicit To: segment, independent
      // of the sender bank, because Deposit filters must always follow destination.
      const toMatches=[...String(info).matchAll(/\bTo\s*:\s*/ig)];
      if(toMatches.length){
        const tm=toMatches[toMatches.length-1];
        const start=(tm.index||0)+tm[0].length;
        let tail=String(info).slice(start);
        const nextFrom=tail.search(/\bFrom\s*:/i);
        if(nextFrom>=0) tail=tail.slice(0,nextFrom);
        destination=clean(tail);
        const dp=destination.split(',').map(clean).filter(Boolean);
        destinationBankName=dp[0]||'';
        destinationAccountNumber=dp[1]||'';
        destinationAccountName=dp.length>2?dp.slice(2).join(', '):'';
      }
    }
    let profilePath='';
    ...
    rows.push({
      key:checkboxName,checkboxName,id2Name,no:clean($(cells[0]).text()),userid,date:clean($(cells[3]).text()),amount,balance,bank,bankName,accountNumber,accountName,info,destination,destinationBankName,destinationAccountNumber,destinationAccountName,
...
function parseMarketHistory(html){
  ...
  $('table tr').each((_,tr)=>{
    const cells=$(tr).find('th,td').map((__,e)=>clean($(e).text())).get();
    if(cells.length<5 || !/^\d+$/.test(cells[0]||'')) return;
    const inputs=$(tr).find('input[type="text"],input:not([type])').map((__,e)=>String($(e).attr('value')||'').trim()).get().filter(x=>/^\d{1,8}$/.test(x));
    const nums=inputs.slice(-3); if(!nums.length)return;
    const period=cells.find((c,i)=>i>1 && /^\d{2,8}$/.test(c))||''; if(!period)return;
    out.push({no:cells[0],date:cells[1]||'',day:cells.find(x=>/^(senin|selasa|rabu|kamis|jumat|sabtu|minggu)$/i.test(x))||'',period,n1:nums[0]||'',n2:nums[1]||'',n3:nums[2]||'',prizeCount:nums.length,calculated:cells.some(x=>/^yes$/i.test(x))?'Yes':''});
...
function marketNumberFields(html,kind){
  const d=formDescriptor(html,kind); if(!d)return [];
  const ignored=/^(periode|period|sar|psr|pasar|submit|tanggal|tgl|day|month|year)$/i;
  return [...new Set(d.names.filter(n=>n&&!ignored.test(n)&&/(nomor|angka|prize|result|keluar)/i.test(n)))].slice(0,3);
}
...
export class UpstreamClient{
  ...
  async decidePendingMoneyBulk({type,keys,decision}){
    const fresh=await this.pendingMoney(type);
    const wanted=new Set(keys);
    const rows=fresh.rows.filter(x=>wanted.has(x.key));
    const missing=keys.filter(k=>!rows.some(x=>x.key===k));
    if(!rows.length) throw new Error('TRANSACTIONS_NOT_FOUND_OR_ALREADY_PROCESSED');
    const form={...fresh.common};
    for(const row of rows){ form[row.checkboxName]='on'; if(row.id2Name) form[row.id2Name]=''; }
    form.submitForm=decision==='accept'?'Accept':'Reject';
    const r=await this.request(fresh.actionPath,{method:'POST',form});
    const text=visibleText(r.html).slice(0,1800);
    const ok=r.ok&&!/(error|gagal|failed|invalid)/i.test(text);
    return {ok,http:r.status,message:text,rows,missing,processedKeys:rows.map(x=>x.key)};
  }
  ...
  async marketState(market){
    ...
    const resultFields=marketNumberFields(resultPage?.r.html||'','result');
    const predictionFields=marketNumberFields(predPage?.r.html||'','prediction');
    const historyCount=Math.max(0,...history.map(x=>Number(x.prizeCount)||0));
    const prizeCount=Math.max(1,Math.min(3,resultFields.length||predictionFields.length||historyCount||1));
    const lastResult=history[0]||null;
    // Nomor Keluar is only actionable while the upstream market page exposes
    // its live result-entry form. A closed market may still expose history,
    // so history alone must never be treated as OPEN.
    const resultDescriptor=formDescriptor(resultPage?.r.html||'','result');
    const marketOpen=Boolean(period && resultDescriptor && resultFields.length);
    // After Nomor Keluar is stored, AGWL5 leaves the newest history row
    // uncalculated until the winner-payment (Hitung) step is executed.
    const pendingCalculation=history.find(x=>x.period && !/^yes$/i.test(String(x.calculated||'')))||null;
    const calculateReady=Boolean(marketOpen && pendingCalculation);
    if(!period && !history.length) throw new Error('MARKET_PAGE_NOT_FOUND');
    return {http:resultPage?.r.status||predPage?.r.status||200,source:resultPage?.path||predPage?.path,market:meta.name,marketId:id,period,prizeCount,predictionPrizeCount:Math.max(1,Math.min(3,predictionFields.length||prizeCount)),resultPrizeCount:Math.max(1,Math.min(3,resultFields.length||historyCount||prizeCount)),marketOpen,calculateReady,pendingCalculation,lastResult,history};
  }
  async marketAction({action,market,period,n1,n2,n3}){
    ...
    if(action==='prediction'){
      ...
      const page=await this.request(target), d=formDescriptor(page.html,'prediction');
      form={...(d?.fields||{}),periode:period,sar:id,submit:'Submit'};
      const names=marketNumberFields(page.html,'prediction'), vals=[n1,n2,n3];
      names.forEach((name,i)=>{if(vals[i])form[name]=vals[i]}); if(!names.length)form.nomor=n1;
      r=await this.request(safeUpstreamAction(d?.action,target),{method:d?.method||method,form});
    }else if(action==='result'){
      target=`/admin_angka13.php?psr=${encodeURIComponent(id)}`;
      const page=await this.request(target), d=formDescriptor(page.html,'result');
      form={...(d?.fields||{}),periode:period,psr:id};
      const names=marketNumberFields(page.html,'result'), vals=[n1,n2,n3];
      names.forEach((name,i)=>{if(vals[i])form[name]=vals[i]});
      if(!names.length){form.angka=n1;if(n2)form.angka2=n2;if(n3)form.angka3=n3}
      const submit=d?.submits?.[0];if(submit?.name)form[submit.name]=submit.value;
      r=await this.request(safeUpstreamAction(d?.action,target),{method:d?.method||method,form});
    }else{
    ...
    if(action==='result'){
      const hit=verify.history.find(x=>String(x.period)===String(period) && x.n1===n1 && (!n2||x.n2===n2) && (!n3||x.n3===n3));
