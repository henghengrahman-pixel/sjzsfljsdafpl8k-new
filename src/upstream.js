...
export function parsePendingMoney(html,type){
  ...
  form.find('tr').each((_,tr)=>{
    ...
    rows.push({
      key:checkboxName,checkboxName,id2Name,userid,date:clean($(cells[3]).text()),amount,balance,bank,bankName,accountNumber,accountName,info,destination,
...
function parseMarkets(html){
  const $=cheerio.load(html), names=new Set();
  const add=v=>{v=clean(v).replace(/[\[\]]/g,'');if(/[A-Z]/i.test(v)&&/^[A-Z0-9][A-Z0-9 _.-]{1,30}$/i.test(v)&&!['POOLS','POKER','MINIGAME'].includes(v.toUpperCase())) names.add(v.toUpperCase())};
  $('select option').each((_,o)=>add($(o).text()||$(o).attr('value')));
  $('a').each((_,a)=>{const t=clean($(a).text());const h=String($(a).attr('href')||'');if(/pasaran|prediksi|menkalah|result/i.test(h)) add(t)});
  const body=clean($('body').text());const m=body.match(/Pools\s*:\s*([^\n]+)/i);if(m) for(const x of m[1].matchAll(/\[([^\]]+)\]/g)) add(x[1]);
  return [...names].filter(x=>x.length>1).sort();
}
...
export class UpstreamClient{
  ...
  async markets(){
    const probes=['/','/admin_prediksifullbb.php','/admin_menkalahbb.php'];let names=new Set(),last=null;
    for(const path of probes){try{const r=await this.request(path);last=r;for(const x of parseMarkets(r.html))names.add(x)}catch(e){if(e.message==='UPSTREAM_SESSION_EXPIRED')throw e}}
    return {http:last?.status||200,markets:[...names].sort().map(name=>({name}))};
  }
  async marketState(market){
    const q=encodeURIComponent(market);const probes=[`/admin_prediksifullbb.php?pasaran=${q}`,`/admin_menkalahbb.php?pasaran=${q}`,`/?pasaran=${q}`];let best=null;
    for(const path of probes){try{const r=await this.request(path);const body=visibleText(r.html);if(body.toUpperCase().includes(market.toUpperCase())){const period=parseMarketPeriod(r.html),history=parseMarketHistory(r.html);if(period||history.length){best={http:r.status,source:path,period,history};break}}}catch(e){if(e.message==='UPSTREAM_SESSION_EXPIRED')throw e}}
    if(!best)throw new Error('MARKET_PAGE_NOT_FOUND');return best;
  }
  async marketAction({action,market,period,n1,n2,n3}){
    const q=encodeURIComponent(market);const candidates=action==='prediction'?[`/admin_prediksifullbb.php?pasaran=${q}`]:[`/admin_menkalahbb.php?pasaran=${q}`,`/admin_prediksifullbb.php?pasaran=${q}`];let page=null,desc=null,source='';
    for(const path of candidates){try{const r=await this.request(path);const d=formDescriptor(r.html,action);if(d){page=r;desc=d;source=path;break}}catch(e){if(e.message==='UPSTREAM_SESSION_EXPIRED')throw e}}
    if(!desc)throw new Error('MARKET_FORM_NOT_FOUND');
    const form={...desc.fields};const names=desc.names;
    const pick=(rx,used=new Set())=>names.find(n=>n&&!used.has(n)&&rx.test(n));
    const periodName=pick(/periode|period/i);if(periodName)form[periodName]=period;
    const marketName=pick(/pasaran|market/i);if(marketName)form[marketName]=market;
    const used=new Set([periodName,marketName].filter(Boolean));
    const explicit=[pick(/^(nomor|angka|prize|result|keluar)1?$|nomor_?1|prize_?1/i,used),pick(/nomor_?2|prize_?2|angka_?2|result_?2|keluar_?2|nomor2/i,used),pick(/nomor_?3|prize_?3|angka_?3|result_?3|keluar_?3|nomor3/i,used)];
    const generic=names.filter(n=>n&&!used.has(n)&&/(nomor|angka|prize|result|keluar)/i.test(n));
    const numberNames=explicit.map((x,i)=>x||generic.filter(g=>!explicit.includes(g))[i]).filter(Boolean).slice(0,3);
    if(numberNames.length<3)throw new Error('MARKET_NUMBER_FIELDS_NOT_FOUND');[n1,n2,n3].forEach((v,i)=>form[numberNames[i]]=v);
    const submit=desc.submits.find(x=>action==='calculate'?/hitung|kirim|submit/i.test(x.value):action==='prediction'?/prediksi|kirim|submit/i.test(x.value):/kirim|nomor|submit/i.test(x.value))||desc.submits[0];if(submit?.name)form[submit.name]=submit.value;
    const target=safeUpstreamAction(desc.action,source);const r=await this.request(target,{method:desc.method==='GET'?'POST':desc.method,form});const text=visibleText(r.html).slice(0,1400);if(!r.ok||/(error|gagal|invalid|failed)/i.test(text))throw new Error(`MARKET_UPSTREAM_REJECTED_HTTP_${r.status}`);
    const verify=await this.marketState(market);return {ok:true,http:r.status,message:text.slice(0,300),verify};
  }
