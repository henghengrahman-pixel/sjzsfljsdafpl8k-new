...
export class UpstreamClient{
  ...
  async marketState(market){
    ...
    return {http:resultPage?.r.status||predPage?.r.status||200,source:resultPage?.path||predPage?.path,market:meta.name,marketId:id,period,prizeCount,predictionPrizeCount:Math.max(1,Math.min(3,predictionFields.length||prizeCount)),resultPrizeCount:Math.max(1,Math.min(3,resultFields.length||historyCount||prizeCount)),lastResult,history};
  }
