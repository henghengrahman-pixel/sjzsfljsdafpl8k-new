...
app.post('/api/markets/action',requireMaster,async(req,res)=>{const p=z.object({action:z.enum(['prediction','result','calculate']),market:marketName,period:z.string().trim().regex(/^\d{1,8}$/),n1:z.string().trim().regex(/^\d{1,8}$/),n2:z.string().trim().regex(/^\d{0,8}$/).default(''),n3:z.string().trim().regex(/^\d{0,8}$/).default('')}).safeParse(req.body);if(!p.success)return res.status(400).json({ok:false,error:'INPUT_INVALID'});const before=await upstream.marketState(p.data.market);if(before.period&&before.period!==p.data.period)return res.status(409).json({ok:false,error:'MARKET_PERIOD_CHANGED',currentPeriod:before.period});const r=await upstream.marketAction(p.data);await audit(db,req,`MARKET_${p.data.action.toUpperCase()}`,p.data.market,{period:p.data.period,n1:p.data.n1,n2:p.data.n2,n3:p.data.n3,ok:r.ok});res.json({ok:true,...r})});
