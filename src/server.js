...
app.get('/api/conversations/:id',requireAdmin,async(req,res)=>{
  ...
  const [meta,msg,state,brain]=await Promise.all([metaPromise,itemsPromise,getConversationState(req.params.id),getConversationBrain(req.params.id)]);
  ...
  res.json({ok:true,chatId:req.params.id,conversation,items,state,brain:brain?.case_brain||{},full,messages:full?null:{hasOlder:msg.hasOlder,beforeId:msg.beforeId,lastId:msg.lastId,limit}});
});
app.get('/api/conversations/:id/messages',requireAdmin,async(req,res)=>{
  ...
  const [meta,msg,state]=await Promise.all([
    pool.query(`SELECT c.chat_id,c.customer_name,c.customer_email,c.status,c.handling_mode,c.handling_state,c.takeover_reason,c.takeover_at,c.last_event_at,c.last_member_event_at,c.last_ai_event_at,c.workflow_type,c.workflow_state,c.visible_in_inbox,CASE WHEN c.last_member_event_at IS NOT NULL AND (c.last_ai_event_at IS NULL OR c.last_member_event_at>c.last_ai_event_at) THEN true ELSE false END AS needs_reply FROM conversations c WHERE c.chat_id=$1 LIMIT 1`,[req.params.id]),
    pool.query(`SELECT id,event_id,sender_type,author_id,text,normalized_text,intent,attachments,created_at FROM messages WHERE chat_id=$1 AND id>$2 ORDER BY id ASC LIMIT $3`,[req.params.id,afterId,limit]),
