...
app.get('/api/session/status',async(req,res)=>{
  ...
  const l=await db.query(`SELECT value FROM app_meta WHERE key='active_login_link'`);
  res.json({ok:true,session:q.rows[0],activeLoginLink:l.rows[0]?.value||''});
});
app.put('/api/session/login-link',requireMaster,async(req,res)=>{
  const p=z.object({url:z.string().trim().url().max(500).refine(v=>v.startsWith('https://'),'HTTPS_REQUIRED')}).safeParse(req.body);
  if(!p.success)return res.status(400).json({ok:false,error:'LINK_INVALID_HTTPS_REQUIRED'});
  await db.query(`INSERT INTO app_meta(key,value,updated_at) VALUES('active_login_link',$1,NOW()) ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value,updated_at=NOW()`,[p.data.url]);
  await audit(db,req,'ACTIVE_LOGIN_LINK_UPDATE','Session',{url:p.data.url});
  res.json({ok:true,url:p.data.url});
});
...
app.post('/api/player/reset-password',async(req,res)=>{const p=z.object({userid:uid,password:z.string().min(6).max(128),confirm:z.string().min(6).max(128)}).refine(x=>x.password===x.confirm).safeParse(req.body);if(!p.success)return res.status(400).json({ok:false,error:'INPUT_INVALID'});const r=await upstream.resetPassword(p.data.userid,p.data.password);await audit(db,req,'RESET_PASSWORD',p.data.userid,{ok:r.ok});if(!r.ok)return res.status(502).json({ok:false,http:r.http,message:r.message});const l=await db.query(`SELECT value FROM app_meta WHERE key='active_login_link'`);res.json({ok:true,http:r.http,message:r.message,resetResult:{userid:p.data.userid,password:p.data.password,loginLink:l.rows[0]?.value||''}})});
