...
app.post('/api/auth/login',limiter,async(req,res)=>{
  const p=z.object({username:z.string().trim().min(1).max(80),password:z.string().min(1).max(256),otp:z.string().regex(/^\d{6}$/)}).safeParse(req.body);
  if(!p.success) return res.status(400).json({ok:false,error:'INPUT_INVALID'});
  const q=await db.query('SELECT id,username,alias,password_hash,totp_secret,role,active FROM app_users WHERE username=$1',[p.data.username]);
  const u=q.rows[0];
  const ok=!!u && u.active && await verifyPassword(p.data.password,u.password_hash) && verifyTotp(p.data.otp,u.totp_secret);
  if(!ok){await new Promise(r=>setTimeout(r,300));return res.status(401).json({ok:false,error:'LOGIN_INVALID'});}
  req.session.regenerate(err=>{
    ...
    req.session.save(async()=>{await audit(db,req,'LOGIN');res.json({ok:true,user:req.session.user,csrf:req.session.csrf});});
  });
});
...
app.post('/api/player/search',async(req,res)=>{const p=z.object({userid:uid}).safeParse(req.body);if(!p.success)return res.status(400).json({ok:false,error:'INPUT_INVALID'});const r=await upstream.searchPlayer(p.data.userid);await audit(db,req,'PLAYER_SEARCH',p.data.userid);res.json({ok:true,...r})});
