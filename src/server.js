  ...
  try{
    ...
    const learningWorker=async()=>{
      try{
        ...
        setTimeout(learningWorker,r.scanned?5000:60000);
      }catch(e){
        console.warn('CS history learning failed',e.message);
        setTimeout(learningWorker,15000);
      }
    };
    setTimeout(learningWorker,30000);
    ...
    const shutdown=async(signal)=>{console.log(`Shutdown ${signal}`);stopPoller();stopHumanBridge();server.close(()=>{});await Promise.allSettled([pool.end(),brainPool===pool?Promise.resolve():brainPool.end()]);};
