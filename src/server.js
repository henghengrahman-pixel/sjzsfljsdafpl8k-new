...
app.post('/api/auth/login',limiter,async(req,res)=>{
  ...
  // First login: show the Authenticator QR/secret only after username + password are valid.
  if(!u.totp_enrolled_at && !p.data.otp){
    const uri=otpauthUrl(u.username,cfg.appName,u.totp_secret);
    const qr=await QRCode.toDataURL(uri);
    return res.json({ok:true,setupRequired:true,username:u.username,totpSecret:u.totp_secret,qr});
  }
