...
export async function backfillHumanLearning(limit=5000){
  ...
  // Runtime history and shared learning may be on different Postgres services.
  // A runtime cursor avoids cross-database JOINs and guarantees old history is
  // eventually scanned exactly once even when BRAIN_DATABASE_URL is shared.
  ...
  for(const m of rows.rows){
    scanned++;
    if(existing.has(`${m.chat_id}\u0000${m.event_id}`)){ skipped++; continue; }
    try{
  ...
  if(rows.rows.length){
    const nextCursor=Number(rows.rows[rows.rows.length-1].id)||cursor;
    await pool.query(`INSERT INTO app_settings(key,value,updated_at) VALUES('learning_backfill_cursor',to_jsonb($1::bigint),now())
      ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value,updated_at=now()`,[nextCursor]);
  }
  return {scanned,created,skipped,staleSkipped,failed,errors,cursor};
}
...
export async function learningStats(){
  const r=await brainPool.query(`SELECT (SELECT count(*)::int FROM learning_examples WHERE source_type='HUMAN_CHAT') human_examples,(SELECT count(*)::int FROM learning_examples WHERE status='APPROVED') approved_examples,(SELECT count(*)::int FROM cs_learning_patterns) patterns,(SELECT count(*)::int FROM cs_learning_patterns WHERE safe_for_reply=true) safe_patterns`);
