...
test('polling uses single-flight recursive timeout and backoff',()=>{assert.match(conv,/if\(state\.listBusy\)return/);assert.match(conv,/setTimeout\(async\(\)=>/);assert.doesNotMatch(conv,/setInterval\(/);assert.match(conv,/pollDelay\*1\.7/)});
test('inbox render is keyed and reorders existing rows',()=>{assert.match(conv,/dataset\.id=id/);assert.match(conv,/list\.append\(row\)/)});
test('message delta endpoint and duplicate suppression exist',()=>{assert.match(server,/\/api\/conversations\/:id\/messages/);assert.match(server,/id>\$2/);assert.match(conv,/known\.has\(id\)/);assert.match(conv,/messageAfterId/)});
...
test('1366 desktop layout has bounded grid and no root horizontal overflow',()=>{assert.match(css,/max-width:100vw/);assert.match(css,/min-width:0/);assert.match(css,/grid-template-columns:300px minmax\(0,1fr\) 280px/)});
